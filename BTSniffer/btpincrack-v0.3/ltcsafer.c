 ...
 */
int saferp_setup(const unsigned char *key, symmetric_key *skey)
{
   unsigned x, y, z;
   unsigned char t[33];
   int keylen = 16;
...
*/
void saferp_ecb_encrypt(const unsigned char *pt, unsigned char *ct, symmetric_key *skey, int modified)
{
   unsigned char b[16];
   int x;
   ...
   if(modified) {
      ...
      b[ 7] ^= pt[ 7]; b[ 8] ^= pt[ 8];
      b[ 9] += pt[ 9]; b[10] += pt[10];
      b[11] ^= pt[11]; b[12] ^= pt[12];
      b[13] += pt[13]; b[14] += pt[14];
      b[15] ^= pt[15];
   }
   ROUND(b,  4);  LT(b, ct);
   ROUND(ct, 6);  LT(ct, b);
...
Ar(u_char *lk, u_char *blk)
{
	...
	saferp_setup(lk, &skey);
	saferp_ecb_encrypt(blk, out, &skey, 0);
...
Ar_(u_char *lk, u_char *blk)
{
	...
	saferp_setup(lk, &skey);
	saferp_ecb_encrypt(blk, out, &skey, 1);
