...
from PM.Gui.Dialogs.Interface import CaptureOptions, InterfaceList, InterfaceDialog
...
#        tbl.attach(self.method, 1, 2, 2, 3, yoptions=gtk.SHRINK)
        ...
        tbl.attach(self.stop_time_combo, 2, 3, 4, 5, yoptions=gtk.SHRINK)
    ...
    def get_options(self):
        ...
        # Currently only has one factor = 0
        if factor == 0:
            factor = 1
        elif factor == 1:
            factor = 60
...
#        self.options.method.set_active(method)
    ...
    def __populate(self):
        ...
        for iface in Backend.find_conn_hcidevs():
            self.if_list.store.append(
                [gtk.STOCK_CONNECT, ''.join(['hci', str(iface.hcidevid)]), iface.btadd]
            )
